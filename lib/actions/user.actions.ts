...
export const getStats = async (clerkId: string) => {
    try {
        ...
        const totalCommentsPosted = await Comment.find({ author: user._id });
        return JSON.parse(
            JSON.stringify({
                followers,
                following,
                ...
                totalCommentsPosted: totalCommentsPosted.length,
            })
        );
    } catch (error) {
        handleError(error);
...
export const getTotalPosts = async (clerkId: string) => {
    try {
        await connectToDatabase();
        const user = await User.findOne({ clerkId });
        if (!user) {
        ...
        const posts = await Post.find({ author: user._id });
        return { status: 200, posts: JSON.parse(JSON.stringify(posts)) };
    } catch (error: any) {
        return { status: 500, message: error.message };
    }
};
...
export const getRecentComments = async (clerkId: string) => {
    try {
        await connectToDatabase();
        const user = await User.findOne({ clerkId });
        if (!user) {
        ...
        const comments = await Comment.find({
            author: user._id,
            $and: [{ post: { $ne: null } }],
        }).populate("post").sort({ createdAt: -1 }).limit(7);
